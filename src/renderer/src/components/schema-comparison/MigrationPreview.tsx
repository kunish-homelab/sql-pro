...
import { useConnectionStore, useQueryTabsStore, useSchemaComparisonStore } from '@/stores';
...
export function MigrationPreview({ className }: MigrationPreviewProps) {
  ...
  const [migrationSQL, setMigrationSQL] = useState<GenerateMigrationSQLResponse | null>(null);
  ...
  useEffect(() => {
    ...
    const generateSQL = async () => {
      ...
      try {
        ...
        setMigrationSQL({
          success: false,
          error: error instanceof Error ? error.message : 'Failed to generate migration SQL',
        });
      } finally {
  ...
  const handleSaveToFile = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.filePath && !result.canceled) {
        ...
        const writeResult = await sqlPro.db.executeQuery({
          connectionId: activeConnectionId || '',
          query: '', // This is a workaround - we need a proper file write API
        });
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-4 rounded-lg border p-3">
          <div className="flex items-center gap-2">
            ...
            />
            <Label htmlFor="reverse-migration" className="cursor-pointer text-sm">
            ...
            {migrationSQL.warnings && migrationSQL.warnings.length > 0 && (
              <Alert>
                ...
                <AlertTitle className="flex items-center justify-between">
                  <span>
                    Warnings ({migrationSQL.warnings.length})
                  </span>
                  <Button
                    variant="ghost"
                    size="sm"
