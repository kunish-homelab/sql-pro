...
describe('FavoritesQuickPanel', () => {
  ...
  describe('rendering as popover', () => {
    it('should render popover trigger button', () => {
      ...
      const triggerButton = screen.getByRole('button', { name: /Favorite Queries/i });
  ...
  describe('rendering as section', () => {
    it('should render section header', () => {
      ...
      expect(screen.getByText('Quick access to your starred queries')).toBeInTheDocument();
    });
  ...
  describe('favorite queries list', () => {
    ...
    it('should display query previews', async () => {
      ...
      expect(screen.getByText(/SELECT \* FROM products WHERE price > 100/)).toBeInTheDocument();
    });
    ...
    it('should truncate long query previews', async () => {
      ...
      const preview = screen.getByText(/SELECT \* FROM products WHERE price > 100/);
  ...
  describe('empty state', () => {
    it('should show empty state when no favorites exist', async () => {
      ...
      expect(screen.getByText('Star queries to add them here')).toBeInTheDocument();
    });
  ...
  describe('query loading', () => {
    it('should call onLoadQuery when query clicked in popover', async () => {
      ...
      const queryButton = screen.getByRole('button', { name: /Favorite Query 1/i });
    ...
    it('should call onLoadQuery when query clicked in section', async () => {
      const user = userEvent.setup();
      render(<FavoritesQuickPanel asSection={true} onLoadQuery={mockOnLoadQuery} />);
    ...
    it('should fallback to query name in tooltip when no description', async () => {
      ...
      const queryButton = screen.getByRole('button', { name: /Favorite Query 2/i });
      expect(queryButton).toHaveAttribute('title', 'Expensive in-stock products');
    });
  });
  ...
  describe('accessibility', () => {
    ...
    it('should have proper title attributes for tooltips', () => {
      ...
      const query1Button = screen.getByRole('button', { name: /Favorite Query 1/i });
      const query2Button = screen.getByRole('button', { name: /Favorite Query 2/i });
  ...
  describe('query preview truncation', () => {
    it('should truncate long query text at 60 characters', async () => {
      const longQuery = {
        id: 'q4',
        name: 'Long Query',
        queryText: 'SELECT * FROM very_long_table_name WHERE column1 = value1 AND column2 = value2 AND column3 = value3',
        description: '',
        isFavorite: true,
        collectionIds: [],
        createdAt: new Date().toISOString(),
