...
describe('SavedQueriesPanel', () => {
  ...
  describe('favorites filter', () => {
    ...
    it('should call setFavoritesOnly when toggle clicked', async () => {
      ...
      const favoriteToggle = screen.getByRole('button', { name: /Show only favorites/i });
    ...
    it('should show empty state when no favorites exist', () => {
      ...
      expect(screen.getByText('Star queries to mark them as favorites')).toBeInTheDocument();
    });
  });
  ...
  describe('collection filter', () => {
    it('should filter queries by selected collection', () => {
      vi.mocked(useSavedQueriesStore).mockReturnValue({
        savedQueries: mockSavedQueries.filter((q) => q.collectionIds.includes('col-1')),
        isLoading: false,
        favoritesOnly: false,
    ...
    it('should show empty state when collection is empty', () => {
      ...
      expect(screen.getByText('No queries in this collection')).toBeInTheDocument();
    });
  });
  ...
  describe('query actions', () => {
    ...
    it('should toggle favorite when star button clicked', async () => {
      ...
      const favoriteButton = starButtons.find((btn) => btn.querySelector('.lucide-star'));
    ...
    it('should copy query to clipboard', async () => {
      ...
      const dropdownButton = moreButtons.find((btn) => btn.querySelector('.lucide-more-vertical'));
