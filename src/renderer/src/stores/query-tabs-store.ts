...
export const useQueryTabsStore = create<QueryTabsState>()(
  persist(
    ...
    {
      name: 'sql-pro-query-tabs-v2',
      partialize: (state) => ({
        tabsByConnection: Object.fromEntries(
          Object.entries(state.tabsByConnection).map(([connId, connState]) => [
            connId,
            {
              tabs: connState.tabs.map((tab) => ({
                ...tab,
              ...
              activeTabId: connState.activeTabId,
              splitLayout: connState.splitLayout,
              activePaneId: connState.activePaneId,
            },
          ])
        ),
