...
describe('Query & Schema Sharing Integration Tests', () => {
  ...
  describe('Query Export/Import Cycle', () => {
    it('should complete full query export/import cycle with file operations', async () => {
      ...
      const originalQuery = {
        ...
        documentation: 'This query is used for monthly user activity reporting.',
      };
      ...
      expect(importResult.query.databaseContext).toBe(originalQuery.databaseContext);
      expect(importResult.query.documentation).toBe(originalQuery.documentation);
    ...
    it('should detect SQL safety warnings during import', async () => {
      ...
      expect(importResult.validation.warnings!.some((w) => w.includes('DROP'))).toBe(true);
    });
  });
  ...
  describe('Schema Export/Import Cycle - JSON Format', () => {
    it('should complete full schema JSON export/import cycle', async () => {
      ...
      const { result: jsonString } = await serializeShareableData(exportResult.data);
    ...
    it('should handle schema with multiple tables and indexes', async () => {
      ...
      expect(importResult.schema.schemas![0].tables[1].foreignKeys).toHaveLength(1);
      expect(importResult.schema.schemas![0].tables[1].foreignKeys[0].table).toBe(
        'products'
      );
    });
  });
  ...
  describe('Schema Export/Import Cycle - SQL Format', () => {
    it('should complete full schema SQL export/import cycle', async () => {
      ...
      expect(importResult.schema.sqlStatements![0]).toContain('CREATE TABLE users');
  ...
  describe('Bundle Export/Import Cycle', () => {
    it('should complete full bundle export/import cycle with multiple queries', async () => {
      ...
      expect(importResult.bundle.databaseContext).toBe(originalBundle.databaseContext);
      expect(importResult.bundle.documentation).toBe(originalBundle.documentation);
      ...
      importResult.bundle.queries.forEach((query, index) => {
        ...
        expect(query.description).toBe(originalBundle.queries[index].description);
  ...
  describe('Full E2E Flow - Complete Sharing Workflow', () => {
    it('should complete entire sharing workflow from export to import', async () => {
      ...
      const { result: exportedData, compressionInfo } = await serializeShareableData(
        exportResult.data
      );
      ...
      expect(importResult.query.databaseContext).toBe(userAQuery.databaseContext);
