...
describe('MigrationGeneratorService', () => {
  ...
  const createComparisonResult = (tableDiffs: TableDiff[]): SchemaComparisonResult => ({
    sourceId: 'source-id',
    sourceName: 'Source',
    ...
    it('should generate CREATE TABLE with foreign keys', () => {
      const tableDiff: TableDiff = {
        ...
        target: createTable(
          'orders',
          'main',
          [createColumn('id', 'INTEGER', false, true), createColumn('user_id', 'INTEGER')],
          [],
          [
            {
              column: 'user_id',
  ...
  describe('generateMigrationSQL - DROP TABLE', () => {
    it('should generate DROP TABLE for removed tables when includeDropStatements is true', () => {
      const tableDiff: TableDiff = {
        ...
        schema: 'main',
        diffType: 'removed',
        source: createTable('old_table', 'main', [createColumn('id', 'INTEGER')]),
        target: null,
      };
      ...
      expect(result.warnings[0]).toContain('Dropping table "old_table" will permanently delete all data');
      expect(result.warnings[0]).toContain('Make sure to backup data before running this migration');
    });
  ...
  describe('generateMigrationSQL - ALTER TABLE ADD COLUMN', () => {
    it('should generate ALTER TABLE ADD COLUMN for added columns', () => {
      const tableDiff: TableDiff = {
        ...
        schema: 'main',
        diffType: 'modified',
        source: createTable('users', 'main', [createColumn('id', 'INTEGER', false, true)]),
        target: createTable('users', 'main', [
          createColumn('id', 'INTEGER', false, true),
    ...
    it('should generate ALTER TABLE ADD COLUMN with NOT NULL and DEFAULT', () => {
      ...
      expect(result.success).toBe(true);
      expect(result.sql).toContain("ALTER TABLE users ADD COLUMN status TEXT NOT NULL DEFAULT 'active'");
    });
  });
  ...
  describe('generateMigrationSQL - Trigger Operations', () => {
    it('should generate CREATE TRIGGER for added triggers', () => {
      ...
      expect(result.success).toBe(true);
      expect(result.sql).toContain('CREATE TRIGGER tr_audit AFTER INSERT ON users BEGIN ...');
    });
  ...
  describe('generateMigrationSQL - SQLite Limitations', () => {
    it('should warn about column removal requiring table recreation', () => {
      const tableDiff: TableDiff = {
        ...
        target: createTable('users', 'main', [createColumn('id', 'INTEGER', false, true)]),
        columnDiffs: [
          {
            name: 'id',
            diffType: 'unchanged',
    ...
    it('should generate table recreation statements for column removal', () => {
      ...
      expect(result.sql).toContain('INSERT INTO users_new (id, name) SELECT id, name FROM users');
  ...
  describe('generateMigrationSQL - Edge Cases', () => {
    ...
    it('should handle schema prefix correctly', () => {
      const tableDiff: TableDiff = {
        ...
        diffType: 'added',
        source: null,
        target: createTable('users', 'temp', [createColumn('id', 'INTEGER', false, true)]),
      };
    ...
    it('should handle multiple table operations in correct order', () => {
      const tableDiffs: TableDiff[] = [
        ...
        // Add a new table
        {
          ...
          diffType: 'added',
          source: null,
          target: createTable('new_table', 'main', [createColumn('id', 'INTEGER', false, true)]),
        },
