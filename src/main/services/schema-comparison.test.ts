...
describe('SchemaComparisonService', () => {
  ...
  describe('compareSchemas - Basic Table Comparisons', () => {
    ...
    it('should detect removed tables', () => {
      const sourceSchemas: SchemaInfo[] = [
        createSchema('main', [
          createTable('users', 'main', [createColumn('id', 'INTEGER', false, true)]),
        ]),
      ];
  ...
  describe('compareSchemas - Column Comparisons', () => {
    it('should detect added columns', () => {
      ...
      const addedColumn = result.tableDiffs[0].columnDiffs?.find((c) => c.name === 'name');
    ...
    it('should detect removed columns', () => {
      ...
      expect(result.tableDiffs[0].diffType).toBe('modified');
      const removedColumn = result.tableDiffs[0].columnDiffs?.find((c) => c.name === 'name');
    ...
    it('should detect column type changes', () => {
      ...
      const modifiedColumn = result.tableDiffs[0].columnDiffs?.find((c) => c.name === 'age');
    ...
    it('should detect nullable changes', () => {
      ...
      const modifiedColumn = result.tableDiffs[0].columnDiffs?.find((c) => c.name === 'name');
    ...
    it('should detect default value changes', () => {
      const sourceSchemas: SchemaInfo[] = [
        createSchema('main', [
          createTable('users', 'main', [createColumn('status', 'TEXT', true, false, null)]),
        ]),
      ];
      const targetSchemas: SchemaInfo[] = [
        createSchema('main', [
          createTable('users', 'main', [createColumn('status', 'TEXT', true, false, "'active'")]),
        ]),
      ];
      ...
      const modifiedColumn = result.tableDiffs[0].columnDiffs?.find((c) => c.name === 'status');
    ...
    it('should detect primary key changes', () => {
      const sourceSchemas: SchemaInfo[] = [
        createSchema('main', [
          createTable('users', 'main', [createColumn('id', 'INTEGER', false, false)]),
        ]),
      ];
      const targetSchemas: SchemaInfo[] = [
        createSchema('main', [
      ...
      const modifiedColumn = result.tableDiffs[0].columnDiffs?.find((c) => c.name === 'id');
      ...
      const sourceSchemas: SchemaInfo[] = [
        createSchema('main', [
          createTable(
            ...
            [{ name: 'idx_users', columns: ['email'], isUnique: false, sql: '' }]
          ),
        ]),
      ];
      const targetSchemas: SchemaInfo[] = [
        createSchema('main', [
          createTable(
            ...
            'main',
            [createColumn('email', 'TEXT'), createColumn('name', 'TEXT')],
            [{ name: 'idx_users', columns: ['email', 'name'], isUnique: false, sql: '' }]
          ),
        ]),
      ];
      ...
      const sourceSchemas: SchemaInfo[] = [
        createSchema('main', [
          createTable(
            ...
            'main',
            [createColumn('email', 'TEXT')],
            [{ name: 'idx_users_email', columns: ['email'], isUnique: false, sql: '' }]
          ),
        ]),
      ];
      const targetSchemas: SchemaInfo[] = [
        createSchema('main', [
          createTable(
            ...
            'main',
            [createColumn('email', 'TEXT')],
            [{ name: 'idx_users_email', columns: ['email'], isUnique: true, sql: '' }]
          ),
        ]),
      ];
  ...
  describe('compareSchemas - Foreign Key Comparisons', () => {
    it('should detect added foreign keys', () => {
      const sourceSchemas: SchemaInfo[] = [
        createSchema('main', [
          createTable('orders', 'main', [createColumn('user_id', 'INTEGER')], [], []),
        ]),
      ];
      const targetSchemas: SchemaInfo[] = [
        createSchema('main', [
  ...
  describe('compareSchemas - Edge Cases', () => {
    ...
    it('should handle composite primary key changes', () => {
      const sourceSchemas: SchemaInfo[] = [
        createSchema('main', [
          createTable(
            'order_items',
            'main',
            [createColumn('order_id', 'INTEGER'), createColumn('item_id', 'INTEGER')],
            [],
            [],
            [],
            ['order_id']
      ...
      expect(result.tableDiffs[0].primaryKeyChanges?.from.sort()).toEqual(['order_id']);
      expect(result.tableDiffs[0].primaryKeyChanges?.to.sort()).toEqual(['item_id', 'order_id']);
    });

    it('should handle multiple schema comparisons', () => {
      const sourceSchemas: SchemaInfo[] = [
        createSchema('main', [createTable('users', 'main', [createColumn('id', 'INTEGER')])]),
        createSchema('temp', [createTable('sessions', 'temp', [createColumn('id', 'INTEGER')])]),
      ];
      const targetSchemas: SchemaInfo[] = [
        createSchema('main', [createTable('users', 'main', [createColumn('id', 'INTEGER')])]),
        createSchema('temp', [createTable('sessions', 'temp', [createColumn('id', 'INTEGER')])]),
      ...
      expect(result.tableDiffs.every((d) => d.diffType === 'unchanged')).toBe(true);
    });
