...
class MigrationGeneratorService {
  ...
  generateMigrationSQL(
    ...
    try {
      const { comparisonResult, reverse = false, includeDropStatements = false } = request;
      ...
      for (const tableDiff of tableDiffs) {
        if (tableDiff.diffType === 'removed' && includeDropStatements) {
        ...
        } else if (tableDiff.diffType === 'modified' && tableDiff.triggerDiffs) {
          ...
          for (const triggerDiff of tableDiff.triggerDiffs) {
            if (triggerDiff.diffType === 'removed' && includeDropStatements) {
              statements.push(this.generateDropTrigger(triggerDiff.source!.name));
      ...
      for (const tableDiff of tableDiffs) {
        if (tableDiff.diffType === 'modified') {
          const needsRecreation = this.checkIfTableNeedsRecreation(tableDiff, includeDropStatements);
          if (needsRecreation) {
      ...
      if (includeDropStatements) {
        for (const tableDiff of tableDiffs) {
          if (tableDiff.diffType === 'removed') {
            statements.push(this.generateDropTable(tableDiff.source!.name, tableDiff.source!.schema));
            warnings.push(
      ...
      for (const tableDiff of tableDiffs) {
        if (tableDiff.diffType === 'modified' && !tablesNeedingRecreation.includes(tableDiff)) {
          if (tableDiff.columnDiffs) {
            for (const columnDiff of tableDiff.columnDiffs) {
              if (columnDiff.diffType === 'added') {
                statements.push(
                  this.generateAddColumn(tableDiff.name, columnDiff.target!, tableDiff.schema)
                );
              } else if (columnDiff.diffType === 'removed' && includeDropStatements) {
                warnings.push(
      ...
      for (const tableDiff of tableDiffs) {
        ...
        } else if (tableDiff.diffType === 'modified' && tableDiff.indexDiffs && !tablesNeedingRecreation.includes(tableDiff)) {
          // Create new/modified indexes for existing table
          for (const indexDiff of tableDiff.indexDiffs) {
            if (indexDiff.diffType === 'added' || indexDiff.diffType === 'modified') {
      ...
      for (const tableDiff of tableDiffs) {
        if (tableDiff.diffType === 'added') {
          ...
          for (const triggerDiff of tableDiff.triggerDiffs) {
            if (triggerDiff.diffType === 'added' || triggerDiff.diffType === 'modified') {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Unknown error generating migration SQL',
      };
    }
  }
  ...
  private reverseTableDiffs(tableDiffs: TableDiff[]): TableDiff[] {
    return tableDiffs.map((tableDiff) => {
      ...
      if (tableDiff.foreignKeyDiffs) {
        reversed.foreignKeyDiffs = this.reverseForeignKeyDiffs(tableDiff.foreignKeyDiffs);
      }
      if (tableDiff.triggerDiffs) {
        reversed.triggerDiffs = this.reverseTriggerDiffs(tableDiff.triggerDiffs);
      }
  ...
  private reverseColumnDiffs(columnDiffs: ColumnDiff[]): ColumnDiff[] {
    return columnDiffs.map((diff) => ({
      ...
      changes: diff.changes
        ? {
            type: diff.changes.type ? { from: diff.changes.type.to, to: diff.changes.type.from } : undefined,
            nullable: diff.changes.nullable ? { from: diff.changes.nullable.to, to: diff.changes.nullable.from } : undefined,
            defaultValue: diff.changes.defaultValue ? { from: diff.changes.defaultValue.to, to: diff.changes.defaultValue.from } : undefined,
            isPrimaryKey: diff.changes.isPrimaryKey ? { from: diff.changes.isPrimaryKey.to, to: diff.changes.isPrimaryKey.from } : undefined,
          }
        : undefined,
    }));
  }
  ...
  private reverseIndexDiffs(indexDiffs: IndexDiff[]): IndexDiff[] {
    return indexDiffs.map((diff) => ({
      ...
      changes: diff.changes
        ? {
            columns: diff.changes.columns ? { from: diff.changes.columns.to, to: diff.changes.columns.from } : undefined,
            isUnique: diff.changes.isUnique ? { from: diff.changes.isUnique.to, to: diff.changes.isUnique.from } : undefined,
          }
        : undefined,
    }));
  }
   ...
   */
  private reverseForeignKeyDiffs(foreignKeyDiffs: ForeignKeyDiff[]): ForeignKeyDiff[] {
    return foreignKeyDiffs.map((diff) => ({
      ...
      changes: diff.changes
        ? {
            referencedTable: diff.changes.referencedTable ? { from: diff.changes.referencedTable.to, to: diff.changes.referencedTable.from } : undefined,
            referencedColumn: diff.changes.referencedColumn ? { from: diff.changes.referencedColumn.to, to: diff.changes.referencedColumn.from } : undefined,
            onDelete: diff.changes.onDelete ? { from: diff.changes.onDelete.to, to: diff.changes.onDelete.from } : undefined,
            onUpdate: diff.changes.onUpdate ? { from: diff.changes.onUpdate.to, to: diff.changes.onUpdate.from } : undefined,
          }
        : undefined,
    }));
  }
  ...
  private reverseTriggerDiffs(triggerDiffs: TriggerDiff[]): TriggerDiff[] {
    return triggerDiffs.map((diff) => ({
      ...
      changes: diff.changes
        ? {
            timing: diff.changes.timing ? { from: diff.changes.timing.to, to: diff.changes.timing.from } : undefined,
            event: diff.changes.event ? { from: diff.changes.event.to, to: diff.changes.event.from } : undefined,
            sql: diff.changes.sql ? { from: diff.changes.sql.to, to: diff.changes.sql.from } : undefined,
          }
        : undefined,
    }));
  }
  ...
  private checkIfTableNeedsRecreation(
    ...
    if (includeDropStatements && tableDiff.columnDiffs?.some((d) => d.diffType === 'removed')) {
      return true;
    }
  ...
  private generateTableRecreation(
    ...
    const fullTableName = schema && schema !== 'main' ? `${schema}.${tableName}` : tableName;
    const fullTempTableName = schema && schema !== 'main' ? `${schema}.${tempTableName}` : tempTableName;
    ...
    const sourceColumns = new Set(tableDiff.source?.columns.map((c) => c.name) || []);
  ...
  private generateCreateTable(table: TableInfo, tableName?: string): string {
    ...
    const fullTableName = table.schema && table.schema !== 'main' ? `${table.schema}.${name}` : name;
   ...
   */
  private generateAddColumn(tableName: string, column: ColumnInfo, schema: string): string {
    const fullTableName = schema && schema !== 'main' ? `${schema}.${tableName}` : tableName;
