...
var isNodeMatchingSelectorFocusable =
  function isNodeMatchingSelectorFocusable$1(options, node) {
    if (
      node.disabled ||
      _isInert(node) ||
      isHiddenInput(node) ||
...
var isNodeMatchingSelectorTabbable = function isNodeMatchingSelectorTabbable$1(
  options,
  node
) {
  if (
...
var activeFocusTraps = {
  ...
  activateTrap: function activateTrap(trapStack, trap) {
    if (trap !== activeFocusTraps.getActiveTrap(trapStack))
...
var createFocusTrap = function createFocusTrap$1(elements, userOptions) {
   ...
   */
  var getOption = function getOption$1(
  ...
  var getReturnFocusNode = function getReturnFocusNode$1(
        ...
        if (config.isolateSubtrees)
          state.adjacentElements.forEach(function (el) {
            if (isEnabled) {
              ...
              el.inert = true;
            } else if (state.alreadyInert.has(el));
            else el.inert = false;
          });
      },
    },
  ...
  watch(
    ...
    (els) => {
      ...
      if (!trap) {
        ...
        const isActive =
          trap === null || trap === void 0 ? void 0 : trap.active;
