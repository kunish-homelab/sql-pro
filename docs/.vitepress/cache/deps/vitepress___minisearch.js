...
var MiniSearch = class MiniSearch {
  ...
  addAllAsync(documents, options = {}) {
    ...
    const { chunk, promise } = documents.reduce(
      ({ chunk: chunk$1, promise: promise$1 }, document, i) => {
  ...
  executeQuery(query, searchOptions = {}) {
    ...
    if (typeof query !== 'string') {
      ...
      const results$1 = query.queries.map((subquery) =>
        this.executeQuery(subquery, options$1)
      );
  ...
  addFieldLength(documentId, fieldId, count, length) {
    ...
    const totalFieldLength =
      (this._avgFieldLength[fieldId] || 0) * count + length;
