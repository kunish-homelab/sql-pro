...
var DOMIterator = class DOMIterator {
  ...
  getContexts() {
    ...
    ctx.forEach((ctx$1) => {
      ...
      if (filteredCtx.indexOf(ctx$1) === -1 && !isDescendant)
        filteredCtx.push(ctx$1);
    });
    return filteredCtx;
  }
  /**
  ...
  compareNodeIframe(node, prevNode, ifr) {
    if (node.compareDocumentPosition(ifr) & Node.DOCUMENT_POSITION_PRECEDING)
      if (prevNode !== null) {
        if (
          prevNode.compareDocumentPosition(ifr) &
    ...
    this.getTextNodes((dict) => {
      dict.nodes.forEach((node) => {
        ...
        while (
          ...
          let pos = match.index;
          if (matchIdx !== 0)
            for (let i = 1; i < matchIdx; i++) pos += match[i].length;
